...
//              {throw('error') }
 ...
 if (angular.isUndefined(config.PNUM)){
    ...
    .controller('MetadataController', ["filepickerService", "$firequeue", "$rootScope", "Collection", "config", "$scope", "$stateParams", "Upload", "$http", "toastr", function(filepickerService, $firequeue, $rootScope, Collection, config, $scope, $stateParams, Upload, $http, toastr){
      var config = $scope.$parent.config || $scope.$parent.$parent.config;
      $scope.config = config;
