...
var app = angular.module('adf.widget.getphd', ['adf.provider',
    'llp.parsetsv', 'roar', 'textSizeSlider',  'LocalStorageModule', 'firebase', 'xeditable', 'ui.tree', 'ngAnimate', 'ngAnnotateText', 'ngDialog', 'ngSanitize', 'pdf', 'toastr', 'mentio', 'diff', 'angularCSS', 'checklist-model', 'angular-md5', 'angular.filter', 'ngFileUpload'
...
$templateCache.put("{widgetsPath}/getphd/src/titleTemplate.html","<div class=panel-heading style=z-index:1;margin:-1px; ng-init=\"coplapse = false\"><h4 class=panel-title style=-webkit-user-select:none;padding:0;><div uib-dropdown uib-keyboard-nav style=z-index:1000000;><a uib-dropdown-toggle class=\"btn btn-xs vcenter pull-left\"><label class=\"fa fa-bars hovergold\" style=\"cursor:pointer;color:beige;filter:drop-shadow(0 0 2.5px rgba(0,0,0,0.5));\"></label></a><ul class=\"uib-dropdown-menu panel r-action\" style=position:absolute;><li uib-tooltip=\"COPY LINK\" tooltip-trigger=mouseenter tooltip-append-to-body=false tooltip-placement=right><a class=hovergold style=\"text-shadow:inset 0 0 1px rgba(0,0,0,0.5);\" clipboard text=roarevent.media on-copied=\"alertify.success(\'copied!\');\" on-error=\"alertify.error(err,\'uh oh!\')\" onclick=\"alertify.success(\'copied!\')\"><i class=\"fa fa-link\" style=margin:5px;></i> COPY LINK</a></li><li uib-tooltip=COLLAPSE tooltip-trigger=mouseenter tooltip-append-to-body=false tooltip-placement=right><a class=hovergold style=\"text-shadow:inset 0 0 1px rgba(0,0,0,0.5);\" title=\"collapse widget\" ng-show=!isCollapsed ng-click=\"roar.collapse();$(\'.issuedocpanel\').css({\'height\':\'auto\'});isCollapsed = true\"><i class=\"fa fa-minus\" style=margin:5px;></i> COLLAPSE</a></li><li uib-tooltip=EXPAND tooltip-trigger=mouseenter tooltip-append-to-body=false tooltip-placement=right><a class=hovergold style=\"text-shadow:inset 0 0 1px rgba(0,0,0,0.5);\" title=\"expand widget\" ng-show=isCollapsed ng-click=\"roar.collapse();$(\'.issuedocpanel\').css({\'height\':\'auto\'});isCollapsed = false\"><i class=\"fa fa-plus\" style=margin:5px;></i> EXPAND</a></li><li uib-tooltip=\"COPY TO CLIPBOARD\" tooltip-trigger=mouseenter tooltip-append-to-body=false tooltip-placement=right><a class=hovergold style=\"text-shadow:inset 0 0 1px rgba(0,0,0,0.5);\" title=\"copy to clipboard\" ng-click=edit(roarevent.id)><i class=\"fa fa-copy\" style=margin:5px;></i> COPY TO CLIPBOARD</a></li><li uib-tooltip=FULLSCREEN tooltip-trigger=mouseenter tooltip-append-to-body=false tooltip-placement=right><a class=hovergold style=\"text-shadow:inset 0 0 1px rgba(0,0,0,0.5);\" title=\"fullscreen widget\" ng-click=openFullScreen(roarevent.id) ng-show=\"options.maximizable || true\"><i class=\"fa fa-expand\" style=margin:5px;></i> FULLSCREEN</a></li><li uib-tooltip=\"POP OUT\" tooltip-trigger=mouseenter tooltip-append-to-body=false tooltip-placement=right><a class=hovergold style=\"text-shadow:inset 0 0 1px rgba(0,0,0,0.5);\" title=\"pop out\" ng-click=openprev><i class=\"fa fa-external-link\" style=margin:5px;></i> POPOUT</a></li><li class=divider></li><li uib-tooltip=CLOSE tooltip-trigger=mouseenter tooltip-append-to-body=false tooltip-placement=right><a class=\"hovergold text-danger\" style=\"text-shadow:inset 0 0 1px rgba(0,0,0,0.5);\" title=\"remove widget\" ng-click=remove() onclick=\"$(this).closest(\'.issuedocpanel\').remove()\"><i class=\"fa fa-close\" style=margin:5px;></i> CLOSE</a></li></ul></div><div class=\"row vcenter\"><small>&nbsp;&nbsp;|&nbsp;&nbsp;</small> <small ng-bind=roarevent.rid ng-click=\"drag = !drag\" ng-class=\"{\':active\':drag}\"></small> <small>&nbsp;&nbsp;|&nbsp;&nbsp;</small> <small style=lead ng-bind=\"roarevent.title || roarevent.date\"></small> <small><em style=margin-left:50px; ng-bind=\"roarevent.date | date\"></em><span class=\"fa {{roarevent.icon || \'fa-info\'}}\" ng-click=\"roarevent.coplapse = !roarevent.coplapse\" style=\"margin: -5px -5px 0px 2.5px;\"></span></small></div></h4><div class=\"card card-block clearfix img img-hover img-shadow\" collection={{roarevent.id}} style=z-index:1;position:absolute;left:99.5%;top:0; ng-class=\"{\'collapse\':!roarevent.coplapse, \'collapse in\':roarevent.coplapse}\" dragable><h6>ID: {{collection.$id}}</h6><p clipboad text=\"{{\'https://lexspace.net/files/uploads/\'+collection.$id+\'.html\'}}\" onsuccess=\"alertify.success(\'copied\')\">{{\'https://lexspace.net/files/uploads/\'+collection.$id+\'.html\'}}</p><form role=form name=form ng-submit=collection.$save()><div class=col-sm-8><div class=form-group><label for=dashboardTitle style=\"color: #006699\">Title</label> <input type=text class=form-control id=dashboardTitle ng-model=collection.title required></div><div class=\"form-group col-sm-6\"><label style=\"color: #006699\">Style</label><br><select ng-model=collection.styleClass ng-options=\"class.value as class.label for class in $root.ROARCLASSES\"></select></div><div class=\"form-group col-sm-6\"><label style=\"color: #006699\">Document Code</label><br><input ng-model=collection.doccode uib-typeahead=\"code.code as code.label for code in DOCNAMES | filter:$viewValue | limitTo:8\" typeahead-wait-ms=100></div><div class=\"form-group col-sm-6\"><label style=\"color: #006699\">Tags</label><br><input ng-model=collection.matches ng-list=,></div><div class=\"form-group col-sm-6\"><label style=\"color: #006699\">Icon</label><br><select name=icon ng-model=collection.icon ng-options=\"icon.value as icon.label for icon in $root.ICONS\"></select></div><div class=\"form-group col-sm-6\"><label style=\"color: #006699\">ID#</label><br><input type=text ng-model=collection.rid placeholder=# ng-model-options=\"{updateOn: \'blur\'}\"></div><div class=\"form-group col-sm-6\"><label style=\"color: #006699\">Date</label><br><input type=text ng-model=collection.date placeholder=YYYY-MM-DD ng-pattern=dddd-dd-dd ng-model-options=\"{updateOn: \'blur\'}\"></div><div class=form-group><label for=description style=\"color: #006699\">Description</label><br><textarea type=text class=form-control id=description ng-model=collection.description ng-model-options=\"{updateOn: \'blur\'}\"></textarea></div><div class=form-group><label for=url style=\"color: #006699\">URL</label><br><input type=text style=\"width: 100%\" ng-model=collection.media placeholder=\"Path to URL\" ng-model-options=\"{updateOn: \'blur\'}\"></div><div class=form-group><label for=thumbnail style=\"color: #006699\">Thumbnail</label><br><input type=text style=\"width: 100%\" ng-model=collection.thumbnail placeholder=\"Path to URL\" ng-model-options=\"{updateOn: \'blur\'}\"></div><div class=form-group><label for=roarlist style=\"color: #006699\">Linked Docs</label><br><ol><li ng-repeat=\"roareventid in collection.roarlist\" style=float:left;><roar-chip id={{roareventid}}></roar-chip></li></ol></div><div class=form-group><label for=navmenu style=\"color: #006699\">Menus <a class=\"fa fa-plus\" ng-click=\"collection.menus[0] = {title: \'Linked Documents\', items: collection.roarlist}\"></a></label><br><ol><li class=list-group-item ng-repeat=\"menu in collection.menus\"><input type=text style=\"width: 100%\" ng-model=menu.title placeholder=\"Path to URL\" ng-model-options=\"{updateOn: \'blur\'}\"><a class=\"fa fa-plus text-success\" ng-click=menu.items.push(collection.id)></a><ul class=list-group><li class=list-group-item ng-repeat=\"menuitem in menu.items\"><input type=text style=\"width: 100%\" ng-model=menuitem placeholder=\"Path to URL\" ng-model-options=\"{updateOn: \'blur\'}\"></li></ul></li></ol></div></div><div class=col-sm-4 style=max-height:175px;><roar-event id={{roarevent.$id}} style=max-height:175px;></roar-event><input type=submit class=\"btn btn-glass btn-primary btn-sm\" value=SAVE><div class=form-group><label for=thumbnail style=\"color: #006699\">Notes</label><br><textarea rows=5 type=text style=\"width: 100%\" ng-model=collection.notes placeholder=\"Enter plaintext notes\" ng-model-options=\"{updateOn: \'default blur\'}\"></textarea></div></div></form></div></div>");
...
$templateCache.put("{widgetsPath}/getphd/src/phd/docheader.html","<div class=\"container-fluid bs two-col-right\"><div class=\"col-xs-4 card card-{{roarevent.styleClass}} col-1\"><img dynamic-background background=url({{background(roarevent.styleClass)}}) background-position=\"0 0;\" background-repeat=no-repeat ng-src=\"{{roarevent.thumbnail || background(roarevent.styleClass)}}\" class=\"img img-responsive img-shadow\" ng-click=\"poodle = !poodle\"><h4>{{roarevent.title}}</h4><p class=card-text><fieldset class=material uib-collapse=!poodle><input type=text placeholder=Search... ng-model=$parent.$$childHead.$$childHead.$$childTail.query><hr><label>Search this document</label></fieldset><label ng-repeat=\"match in roarevent.matches track by $index | groupBy: match\" class=\"fa fa-tag\">&nbsp;<a ng-click=getRelated(match)>{{match}}</a></label></p><ul uib-collapse=!poodle><li ng-repeat=\"(key, p) in roarevent.roarlist\" ffbase={{key}} class=\"btn rlabel btn-glass btn-{{item.styleClass}} badger badger-{{item.styleClass}}\" data-badger={{item.rid}} ng-href=\"{{\'https://lexspace.net/files/uploads/\'+key+\'.html\'}}\" pop><p>{{item.title}} <a pop ng-href={{item.media}}><span class=\"showonhover fa fa-external-link-square\"></span></a></p></li></ul><ul><li class=\"card img-shadow\" ng-repeat=\"a in annotationsAsFlatList($parent.$$childHead.$$childHead.$$childTail.roarevent.pages) track by $index\" style=\"border: 2.5px solid {{a.type}};\">{{a.data.comment}} <span ng-if=a.data.points>({{a.data.points}}<ng-pluralize count=a.data.points when=\"{\'-1\': \'point\', \'1\': \'point\', \'other\': \'points\'}\"></ng-pluralize>)</span></li></ul></div><div class=\"col-xs-8 col-2\"><div class=\"bs-callout bs-callout-reverse bs-callout-{{roarevent.styleClass}}\"><h4>{{roarevent.title}}</h4><p ng-bind=roarevent.description></p><p ng-if=roardate>Dated {{roarevent.date | date }}</p><cite class=pull-right>{{roarevent.filename || roarevent.$id}}&nbsp;&nbsp;<a pop ng-href={{roarevent.media}} target=fframe><i class=\"fa fa-external-link\"></i></a></cite></div><div data-ng-transclude></div></div></div>");
