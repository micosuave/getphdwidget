...
$templateCache.put("{widgetsPath}/getphd/src/phd/docheader.html","<div class=\"container-fluid card bs two-col-right\"><div class=\"col-xs-4 card card-{{roarevent.styleClass}} col-1\" style=max-width:33%;><img dynamic-background background=url({{background(roarevent.styleClass)}}) background-position=\"0 0;\" background-repeat=no-repeat ng-src=\"{{roarevent.thumbnail || background(roarevent.styleClass)}}\" class=\"img img-responsive img-shadow img-thumbnail\" ng-click=\"poodle = !poodle\"><h4>{{roarevent.title}}</h4><p class=card-text><label ng-repeat=\"match in roarevent.matches track by $index | groupBy: match\" class=\"fa fa-tag\">&nbsp;<a ng-click=getRelated(match)>{{match}}</a>&nbsp;</label></p><div class=\"well list-group\" uib-collapse=!poodle><div class=card-header><h4 class=\"card-title fa fa-link\">Linked Documents</h4></div><div ng-repeat=\"vent in roarevent.roarlist\" ffbase=\"{{vent.$id || vent.$value || vent}}\"><roar-chip class=list-group-item id=\"{{vent.$id || vent.$value || vent}}\"></roar-chip><div class=\"well list-group\"><div ng-repeat=\"tent in item.roarlist\"><roar-chip class=list-group-item id=\"{{tent.$id || tent.$value || tent}}\"></roar-chip></div></div></div></div><ul ng-if=annotationsAsFlatList($parent.$$childHead.$$childHead.$$childTail.roarevent.pages)><li class=\"card img-shadow\" ng-repeat=\"a in annotationsAsFlatList($parent.$$childHead.$$childHead.$$childTail.roarevent.pages) track by $index\" style=\"border: 2.5px solid {{a.type}};\">{{a.data.comment}} <span ng-if=a.data.points>({{a.data.points}}<ng-pluralize count=a.data.points when=\"{\'-1\': \'point\', \'1\': \'point\', \'other\': \'points\'}\"></ng-pluralize>)</span></li></ul><table-of-contents root={{roarevent.$id}} ng-if=roarevent.roarlist uib-collapse=!poodle></table-of-contents></div><div class=\"col-xs-8 col-2\"><div class=\"bs-callout bs-callout-reverse bs-callout-{{roarevent.styleClass}}\"><h4>{{roarevent.title}}</h4><p ng-bind=roarevent.description></p><p ng-if=roardate>Dated {{roarevent.date | date }}</p><cite class=\"pull-right clearfix\">{{roarevent.filename || roarevent.file.name || roarevent.name || roarevent.$id}}&nbsp;&nbsp;<a pop ng-href={{roarevent.mediaUrl}} target=fframe><i class=\"fa fa-external-link\"></i></a></cite></div><div data-ng-transclude style=height:500px;overflow:scroll;></div></div></div>");
