...
$templateCache.put("{widgetsPath}/getphd/src/titleTemplate.html","<div class=\"card bs-callout bs-callout-{{roarevent.styleClass || \'primary\'}}\" style=z-index:1;padding:0px;><h4><span>{{config.title || roarevent.title || \'\'}}</span><br><div class=row style=width:100%;><small>{{(roarevent.date | date)||\'\'}}</small> <span class=pull-right style=position:absolute;right:5px;font-size:12px;><a title=notes ng-click=note(roarevent)><i class=\"fa fa-pencil\" style=margin:5px;></i></a><a title=\"reload widget content\" ng-click=reload()><i class=\"fa fa-refresh\" style=margin:5px;></i></a> <a title=\"change widget location\" class=adf-move><i class=\"fa fa-arrows\" style=margin:5px;></i></a> <a title=\"collapse widget\" ng-show=\"options.collapsible && !widgetState.isCollapsed\" ng-click=\"widgetState.isCollapsed = !widgetState.isCollapsed\"><i class=\"fa fa-minus\" style=margin:5px;></i></a> <a title=\"expand widget\" ng-show=\"options.collapsible && widgetState.isCollapsed\" ng-click=\"widgetState.isCollapsed = !widgetState.isCollapsed\"><i class=\"fa fa-plus\" style=margin:5px;></i></a> <a title=\"copy to clipboard\" ng-click=edit(roarevent.id)><i class=\"fa fa-copy\" style=margin:5px;></i></a> <a title=\"fullscreen widget\" ng-click=openFullScreen(roarevent.id) ng-show=\"options.maximizable || true\"><i class=\"fa fa-expand\" style=margin:5px;></i></a> <a title=\"remove widget\" ng-click=remove() onclick=$(this).parent().parent().parent().parent().parent().remove()><i class=\"fa fa-close\" style=margin:5px;></i></a></span></div></h4></div>");
...
$templateCache.put("{widgetsPath}/getphd/src/phd/Pages.html","<div ng-repeat=\"page in roarevent.pages\" class=\"card card-block\" style=line-height:1.5;font-size:14px;><ng-annotate-text text=page class=\"eventdigest card\" style=\"background-color: #fff;padding: 1rem;border-radius: 0.2rem;\" annotations=roarevent.annotations[$index] readonly=false popup-controller=\"\'AnnotationController\'\" popup-template-url=\"\'/llp_core/modules/roarmap/partial/roarmap.detail.tpl/roarmap.annotation.tpl.html\'\" tooltip-controller=\"\'AnnotationController\'\" tooltip-template-url=\"\'/llp_core/modules/roarmap/partial/roarmap.detail.tpl/roarmap.annotation-tooltip.tpl.html\'\" on-annotate=onAnnotate on-annotate-delete=onAnnotateDelete on-annotate-error=onAnnotateError on-popup-show=onPopupShow on-popup-hide=onPopupHide popup-offset=50></ng-annotate-text><ul><li ng-repeat=\"a in annotationsAsFlatList(roarevent.annotations[$index]) track by $index\">{{a.data.comment}} <span ng-if=a.data.points>({{a.data.points}}<ng-pluralize count=a.data.points when=\"{\'-1\': \'point\', \'1\': \'point\', \'other\': \'points\'}\"></ng-pluralize>)</span></li></ul><footer style=display:flex;justify-content:center;><p style=align-self:center;color:#444;>{{$index + 1}}</p></footer></div>");
$templateCache.put("{widgetsPath}/getphd/src/phd/citation.html","<div class=\"bs-callout bs-callout-NOA\"><h4 class=card-title>{{p.title}}</h4><small class=\"text-muted>\">{{p.application}}</small><p>{{p.description}}</p><img ng-src=/patents/{{$parent.citation}}/preview class=\"img img-responsive img-shadow pull-right\" style=width:100px;height:100px;></div>");
