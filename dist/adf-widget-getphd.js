...
.controller('MetadataController', ["filepickerService", "$firequeue", "$rootScope", "Collection", "config", "$scope", "$stateParams", "Upload", "$http", "toastr", "DOCNAMES", function(filepickerService, $firequeue, $rootScope, Collection, config, $scope, $stateParams, Upload, $http, toastr, DOCNAMES) {
        ...
        var callback = function(data) {
            ...
            Upload.upload({ url: '/upload/', data: { file: Upload.rename(blob, appnum + '.zip') } })
                .then(function(resp) {
                    ...
                    if (progress <= 40) { main.progresstype = 'danger'; } else if (progress > 40 && progress < 66) { main.progresstype = 'warning'; } else if (progress > 97) { main.progresstype = 'success'; } else { main.progresstype = 'info'; }
    ...
    return extractzip;
}]).directive('metadataViewer', ["Collection", function(Collection) {
    return {
        restrict: 'E',
        templateUrl: '{widgetsPath}/getphd/src/metadata.html',
        scope: {},
...
$templateCache.put("{widgetsPath}/getphd/src/claimtreetab.html","<blockquote cite=\"{{phd.patent.media || patent.media}}\" style=\"margin: 25px;margin-top:35px;\"><h4><strong ng-bind=tree.title></strong> - <small>{{tree.claim_total}} Claims</small></h4><p ng-bind-html=\"tree.abstract | highlight: query | trustAsHTML\"></p><cite>{{phd.patent.filename || patent.filename || config.PNUM}} <a class=\"fa fa-external-link fa-border\" target=pop ng-href=\"{{\'/files/public/uspto/patents/\'+ (config.PNUM || patent.id || tree.id)+\'/\'+(config.PNUM || patent.id || tree.id) + \'.pdf\'}}\"></a></cite></blockquote><div class=\"row showscroll\" style=\"margin: 5px 5px;overflow-x:scroll;\"><a ng-repeat=\"link in tree.drawings\" ng-href={{link}} pop ng-click=main.pop(link) class=btn style=margin:2px;><img ng-src={{tree.thumbnails[$index]}} class=\"img img-shadow\"></a></div><div class=row><div class=col-sm-6><fieldset class=material><label>Search</label> <input id=patterninput ng-model=query type=search style=margin:10px; ng-model-options=\"{updateOn: \'default blur\',debounce: {\'default\':250,\'blur\':0}}\"><hr></fieldset><a style=position:absolute;right:0;top:4; class=\"btn fa fa-close\" ng-click=\"query = null\"></a><d3pendingtree id=\"a{{ config.PNUM || config.IPAYEAR + config.IPANUM}}\" class=\"{{query}} resizable ui-resizable\" tree=tree pattern={{query}} patent=\"{{config.PNUM || config.IPAYEAR + config.IPANUM}}\" style=width:100%;></d3pendingtree></div><div class=col-sm-6><div id=info></div><div ui-tree=treeOptions class=claimtree><ol ui-tree-nodes max-depth=6 ng-model=tree.claims><li ui-tree-node class=\"card card-block\" ng-repeat=\"node in tree.claims\" ng-include=\"\'claim_renderer.html\'\" style=padding:0.25in;padding-bottom:0.1rem; ng-hide=\"!treeFilter(node, query, supportedFields) && dim\"></li></ol></div><script type=text/ng-template id=claim_renderer.html><div ui-tree-handle class=\"tree-node tree-node-content\"> <div class=\"tree-node-content flextoprow \" style=\"position:relative;\"> <a class=\"btn btn-xs pull-left\" data-nodrag ng-click=\"toggle(this)\" ng-if=\"node.children && (node.children.length > 0)\"> <span class=\"fa \" ng-class=\"{\'fa-chevron-right\': collapsed, \'fa-chevron-down\': !collapsed}\" style=\"color:steelblue;transition:all 0.25s ease;\"></span> </a> <div ng-bind-html=\"node.text | highlight: query \" ng-class=\"{\'filtered-out\':(!treeFilter(node, query, supportedFields) && dim)}\"></div> </div> </div> <ol ui-tree-nodes=\"\" ng-model=\"node.children\" ng-class=\"{\'hidden\': collapsed}\" style=\"\" class=\"claimtree\" ng-init=\"collapsed = false\"> <li class=\"card card-block img-shadow\" ng-repeat=\"node in node.children\" ui-tree-node ng-include=\"\'claim_renderer.html\'\" style=\"padding:0.25in;padding-bottom:0.1rem;\" ng-hide=\"!treeFilter(node, query, supportedFields) && dim\"> </li> </ol></script></div></div>");
...
$templateCache.put("{widgetsPath}/getphd/src/fulltext.html","<div collection={{config.id}}><doc-header class=\"container-fluid card two-col-right\" roarid={{config.id}}><a href=# editable-textarea=collection.notes e-rows=15 e-cols=80><p>{{ collection.notes || collection.plaintext || collection.text || collection.description || collection.content || \'no notes or text\' }}</p></a></doc-header><div matches=matches getpdftext={{collection.$id}} pdf-data={{collection.$id}} collection={{config.id}} class=\"card card-block\"></div><div ng-annotate-text=collection.content></div><div markdown-to-html=collection.notes></div><textarea ng-model=collection.notes ck-inline=ckdefault ng-change=collection.$save()></textarea></div>");
...
$templateCache.put("{widgetsPath}/getphd/src/metadata.html","<div class=\"card clearfix img img-hover img-shadow\" collection={{config.id}}><form role=form name=form ng-submit=collection.$save()><div class=container><div class=col-sm-7><div class=form-group><h6>ID: {{collection.$id}}</h6><p class=btn clipboard text=\"\'/files/uploads/\'+collection.$id+\'.html\'\" on-click=\"alertify.success(\'/files/uploads/\'+{{collection.$id}}+\'.html\')\" style=padding:0px;>\'{{\'/files/uploads/\'+collection.$id+\'.html\'}}</p></div><div class=form-group style=height:40px;><div class=\"form-group col-sm-6\" style=padding:0px;><label style=\"color: #006699\">Date</label><br><input type=text ng-model=collection.date placeholder=YYYY-MM-DD ng-pattern=dddd-dd-dd ng-model-options=\"{updateOn: \'blur\'}\"></div><div class=\"form-group col-sm-6\"><label style=\"color: #006699\">ID#</label><br><input type=text ng-model=collection.rid placeholder=# ng-model-options=\"{updateOn: \'blur\'}\"></div></div><div class=form-group><label for=dashboardTitle style=\"color: #006699\">Title</label> <input type=text style=\"width: 100%\" id=dashboardTitle ng-model=collection.title placeholder=Title required> <input type=text style=\"width: 100%\" id=dashboardTitle2 ng-model=collection.title2 placeholder=\"Line 2\"></div><div class=form-group><label for=description style=\"color: #006699\">Description</label><br><textarea type=text rows=5 class=form-control id=description ng-model=collection.description ng-model-options=\"{updateOn: \'blur\'}\"></textarea></div><div class=form-group><label for=long_cite style=\"color: #006699\">Long Citation</label><br><input type=text style=\"width: 100%\" ng-model=collection.citation placeholder=\"Long Citation\" ng-model-options=\"{updateOn: \'blur\'}\"></div><div class=form-group><label for=short_cite style=\"color: #006699\">Short Citation</label><br><input type=text style=\"width: 100%\" ng-model=collection.short_cite placeholder=\"Short Citation\" ng-model-options=\"{updateOn: \'blur\'}\"></div><div class=form-group><label for=thumbnail style=\"color: #006699\">Notes</label><br><textarea rows=15 type=text style=\"width: 100%\" ng-model=collection.notes placeholder=\"Enter plaintext/markdown notes\" ng-model-options=\"{updateOn: \'default blur\'}\" highlight=format highlight-onchange=list($markers) class=form-control></textarea></div><div class=form-group><label for=roarlist style=\"color: #006699\">Linked Docs</label><br><div smartpanel={{collection.$id}}></div></div><div class=form-group><label for=menus style=color:var(--applicantblue)>{{collection.menus.length}} Menus <button class=\"btn btn-success btn-xs text-success fa fa-plus\" ng-click=\"collection.menus[(collection.menus.length||0)]={\'title\': \'NewMenu\', \'icon\': \'fa-circle\', \'items\': {\'0\':\'ID\'}}\"></button></label><ul ng-repeat=\"menu in collection.menus\"><label class=\"fa {{menu-icon}}\"><i class=\"fa {{menu.icon}}\"></i>&nbsp;&nbps;{{menu.title}}</label> <input type=text ng-model=menu.title placeholder=Title> <button class=\"fa fa-plus text-success\" ng-click=\"menu.items[menu.items.length || 0]=\'7654321\'}\"></button><select ng-model=menu.icon ng-options=\"icon.value as icon.label for icon in $root.ICONS\"></select><li ng-repeat=\"link in menu.items track by $index\"><input type=text ng-model=link ng-if=!poodle ng-model-options=\"{updateOn: \'default blur\'}\" placeholder=LINK_ID_#> <button class=\"btn btn-xs text-success fa fa-plus\" ng-click=\"menu.items.push({title:\'NewLink\',url:\'//\',target:\'_self\'})\"></button> <span ffbase={{link}} class=\"badge badge fa {{item.icon}}\">{{item.title}}</span> <button class=\"btn btn-xs text-info fa fa-chevron-right\" ng-click=\"poodle = !poodle; link = {\'title\':\'NewLink\',\'url\':\'//\',\'target\':\'_self\'};menu.items[$index]=link;\"></button> <input type=text ng-if=\"link.title || poodle\" ng-model=link.title placeholder=title> <input type=text ng-if=\"link.url || poodle\" ng-model=link.url placeholder=URL><select ng-if=\"link.target || poodle\" ng-model=link.target placeholder=target><option label></option><option label=_blank value=_blank></option><option label=_self value=_self></option><option label=_parent value=_parent></option><option label=_top value=_top></option><option label=fframe value=fframe selected></option><option label=pop value=pop></option></select><button class=\"btn btn-xs text-danger fa fa-close\" ng-click=\"menu.items[$index]=null;cleanUp(collection)\"></button></li></ul></div></div><div class=col-sm-5><div class=form-group style=height:175px;><roar-event id={{collection.$id}} style=max-height:175px;><uib-rating ng-model=collection.rate max=5></uib-rating></roar-event></div><div class=form-group><label style=\"color: #006699\">Icon</label><br><select name=icon ng-model=collection.icon ng-options=\"icon.value as icon.label for icon in $root.ICONS\"></select><input type=submit class=\"btn btn-glass btn-primary btn-md pull-right\" value=SAVE></div><div class=form-group><label style=\"color: #006699\">Style</label><br><select ng-model=collection.styleClass ng-options=\"class.value as class.label for class in $root.ROARCLASSES\"></select></div><div class=form-group><label for=sortOrder style=color:#006699>Sort Order</label><br><input type=number style=width:100%; placeholder=# id=sortOrder ng-model=collection.sortOrder></div><div class=form-group><label style=\"color: #006699\">Document Code</label><br><input style=\"width: 100%\" ng-model=collection.doccode uib-typeahead=\"code.code as code.label for code in DOCNAMES | filter:$viewValue | limitTo:8\" typeahead-wait-ms=100></div><div class=form-group><label style=\"color: #006699\">Tags</label><br><input style=\"width: 100%\" ng-model=collection.matches ng-list=,></div><div class=form-group><label style=\"color: #006699\">Name</label><br><input type=text style=\"width: 100%\" ng-model=collection.name placeholder=name ng-model-options=\"{updateOn: \'blur\'}\"></div><div class=form-group><label style=\"color: #006699\">FileName</label><br><input type=text style=\"width: 100%\" ng-model=collection.filename placeholder=\"file name\" ng-model-options=\"{updateOn: \'blur\'}\"></div><div class=form-group><label style=color:#006699>WebSource Original URL</label><br><input type=text style=\"width: 100%\" ng-model=collection.originalUrl placeholder=/path/to/url ng-model-options=\"{updateOn: \'blur\'}\" ng-readonly=\"collection.originalUrl !== (\'\'||null)\"></div><div class=form-group><label for=url style=\"color: #006699\" clipboard text=\"\'https://mozilla.github.io/pdf.js/web/viewer.html?file=\'\" on-click=\"alertify.success(\'https://mozilla.github.io/pdf.js/web/viewer.html?file=\')\" ng-click=\"collection.mediaUrl = \'https://mozilla.github.io/pdf.js/web/viewer.html?file=\'+collection.mediaUrl\">URL</label><br><input type=text style=\"width: 100%\" ng-model=collection.mediaUrl placeholder=\"Path to URL\" ng-model-options=\"{updateOn: \'blur\'}\" id=url></div><div class=form-group><label for=thumbnail style=\"color: #006699\">Thumbnail</label><br><input type=text style=\"width: 100%\" ng-model=collection.thumbnail placeholder=\"Path to URL\" ng-model-options=\"{updateOn: \'blur\'}\" id=thumbnail></div><div class=form-group><label for=background style=\"color: #006699\">Background Image</label><br><input type=text style=\"width: 100%\" ng-model=collection.background placeholder=\"Path to URL\" ng-model-options=\"{updateOn: \'blur\'}\" id=background></div><div class=form-group><label for=footer style=\"color: #006699\">Footer</label><br><input type=text style=\"width: 100%\" ng-model=collection.footer placeholder=footer ng-model-options=\"{updateOn: \'blur\'}\" id=footer></div><div class=form-group style=padding:0px;><label for=roarlist style=\"color: #006699\">Linked Docs <i class=badge>{{roare.length}}</i></label><ul ng-scrollbar style=\"height:300px;overflow-y:scroll;list-style: none;\"><li ng-repeat=\"roareventid in collection.roarlist as roare\" ffbase={{roareventid}}><roar-chip id={{roareventid}} roarevent=item></roar-chip></li></ul></div><div class=form-group><label for=links style=color:var(--applicantblue)>{{collection.links.length}} Links <button class=\"btn btn-success btn-xs fa fa-plus\" ng-click=\"collection.links[(collection.links.length || 0)]={\'title\': \'NewLink\', \'url\': \'//\', \'target\':\'_blank\'}\"></button></label><ul><li ng-repeat=\"link in collection.links track by $index\"><input type=text ng-model=link.title placeholder=title> <input type=text ng-model=link.url placeholder=URL><select ng-model=link.target placeholder=target><option label></option><option label=_blank value=_blank></option><option label=_self value=_self></option><option label=_parent value=_parent></option><option label=_top value=_top></option><option label=fframe value=fframe selected></option><option label=pop value=pop></option></select></li></ul></div></div></div></form></div>");
...
$templateCache.put("{widgetsPath}/getphd/src/titleTemplate.html","<div class=panel-heading style=\"z-index:1;margin:-5px 0px;padding:0px;filter:drop-shadow(0px, 5px, 10px, rgba(0,0,0,0.75));\" ng-init=\"coplapse = false\"><h4 class=panel-title style=z-index:1;margin:0px;padding:0px;><div uib-dropdown uib-keyboard-nav><a uib-dropdown-toggle class=\"btn btn-xs vcenter pull-left no-arrow\"><label class=\"fa fa-bars hovergold\" style=\"cursor:pointer;filter:drop-shadow(0 0 2.5px rgba(0,0,0,0.5));\"></label></a><ul class=\"uib-dropdown-menu panel r-action\" style=\"position:absolute;width:280px;box-shadow: 5px 5px 10px rgba(0,0,0,0.5);font-size:14px;\"><li uib-tooltip=\"COPY LINK\" tooltip-trigger=mouseenter tooltip-append-to-body=false tooltip-placement=right><a class=hovergold style=\"text-shadow:inset 0 0 1px rgba(0,0,0,0.5);\" clipboard text=roarevent.media on-copied=\"alertify.success(\'copied!\');\" on-error=\"alertify.error(err,\'uh oh!\')\" onclick=\"alertify.success(\'copied!\')\"><i class=\"fa fa-link\" style=margin:5px;></i> COPY LINK</a></li><li uib-tooltip=\"COPY LEXPAD ID\" tooltip-trigger=mouseenter tooltip-append-to-body=false tooltip-placement=right><a class=hovergold style=\"text-shadow:inset 0 0 1px rgba(0,0,0,0.5);\" clipboard text=roarevent.$id on-copied=\"alertify.success(\'copied!\');\" on-error=\"alertify.error(err,\'uh oh!\')\" onclick=\"alertify.success(\'copied!\')\"><i class=\"fa fa-info\" style=margin:5px;></i> COPY LEXPAD ID</a></li><li uib-tooltip=COLLAPSE tooltip-trigger=mouseenter tooltip-append-to-body=false tooltip-placement=right><a class=hovergold style=\"text-shadow:inset 0 0 1px rgba(0,0,0,0.5);\" title=\"collapse widget\" ng-show=!isCollapsed ng-click=\"isCollapsed = true\" onclick=\"$(this).closest(\'.issuedocpanel\').addClass(\'closed\')\"><i class=\"fa fa-minus\" style=margin:5px;></i> COLLAPSE</a></li><li uib-tooltip=EXPAND tooltip-trigger=mouseenter tooltip-append-to-body=false tooltip-placement=right><a class=hovergold style=\"text-shadow:inset 0 0 1px rgba(0,0,0,0.5);\" title=\"expand widget\" ng-show=isCollapsed ng-click=\"isCollapsed = false\" onclick=\"$(this).closest(\'.issuedocpanel\').removeClass(\'closed\')\"><i class=\"fa fa-plus\" style=margin:5px;></i> EXPAND</a></li><li uib-tooltip=\"COPY TO CLIPBOARD\" tooltip-trigger=mouseenter tooltip-append-to-body=false tooltip-placement=right><a class=hovergold style=\"text-shadow:inset 0 0 1px rgba(0,0,0,0.5);\" title=\"copy to clipboard\" ng-click=edit(roarevent.id)><i class=\"fa fa-copy\" style=margin:5px;></i> COPY TO CLIPBOARD</a></li><li uib-tooltip=FULLSCREEN tooltip-trigger=mouseenter tooltip-append-to-body=false tooltip-placement=right><a class=hovergold style=\"text-shadow:inset 0 0 1px rgba(0,0,0,0.5);\" title=\"fullscreen widget\" ng-click=\"openFullscreen($(this).closest(\'.issuedocpanel\'))\" ng-show=\"options.maximizable || true\"><i class=\"fa fa-expand\" style=margin:5px;></i> FULLSCREEN</a></li><li uib-tooltip=\"POP OUT\" tooltip-trigger=mouseenter tooltip-append-to-body=false tooltip-placement=right><a class=hovergold style=\"text-shadow:inset 0 0 1px rgba(0,0,0,0.5);\" title=\"pop out\" ng-click=openpreview(roarevent)><i class=\"fa fa-external-link\" style=margin:5px;></i> POPOUT</a></li><li class=divider></li><li uib-tooltip=CLOSE tooltip-trigger=mouseenter tooltip-append-to-body=false tooltip-placement=right><a class=\"hovergold text-danger\" style=\"text-shadow:inset 0 0 1px rgba(0,0,0,0.5);\" title=\"remove widget\" ng-click=remove() onclick=\"$(this).closest(\'.issuedocpanel\').remove()\"><i class=\"fa fa-close\" style=margin:5px;></i> CLOSE</a></li></ul></div><div class=\"row vcenter\"><small style=\"font-size: 18px;\">&nbsp;|&nbsp;&nbsp;{{roarevent.rid||\'-\'}}&nbsp;&nbsp;|&nbsp;</small><small ng-bind=\"roarevent.title || roarevent.name | limitTo: 50\" style=font-size:18px;text-overflow:ellipsis;></small> <em style=\"font-size: 12px;\" ng-bind=\"roarevent.date | date\"></em> <span class=\"fa {{roarevent.icon || \'fa-info\'}} hovergold\" ng-click=\"roarevent.coplapse = !roarevent.coplapse\"></span> <span class=\"fa fa-close hovergold\" ng-click=remove() onclick=\"$(this).closest(\'.issuedocpanel\').remove()\"></span></div></h4></div>");
$templateCache.put("{widgetsPath}/getphd/src/titleTemplateDropdown.html","<div uib-dropdown uib-keyboard-nav style=z-index:1000000;><a uib-dropdown-toggle class=\"btn btn-xs vcenter pull-left no-arrow\"><label class=\"fa fa-bars hovergold\" style=\"cursor:pointer;filter:drop-shadow(0 0 2.5px rgba(0,0,0,0.5));color: #fff;\"></label></a><ul class=\"uib-dropdown-menu panel r-action\" style=\"position:absolute;width:280px;box-shadow: 5px 5px 10px rgba(0,0,0,0.5);\"><li uib-tooltip=\"COPY LINK\" tooltip-trigger=mouseenter tooltip-append-to-body=false tooltip-placement=right><a class=hovergold style=\"text-shadow:inset 0 0 1px rgba(0,0,0,0.5);\" clipboard text=roarevent.media on-copied=\"alertify.success(\'copied!\');\" on-error=\"alertify.error(err,\'uh oh!\')\" onclick=\"alertify.success(\'copied!\')\"><i class=\"fa fa-link\" style=margin:5px;></i> COPY LINK</a></li><li uib-tooltip=\"COPY LEXPAD ID\" tooltip-trigger=mouseenter tooltip-append-to-body=false tooltip-placement=right><a class=hovergold style=\"text-shadow:inset 0 0 1px rgba(0,0,0,0.5);\" clipboard text=roarevent.$id on-copied=\"alertify.success(\'copied!\');\" on-error=\"alertify.error(err,\'uh oh!\')\" onclick=\"alertify.success(\'copied!\')\"><i class=\"fa fa-info\" style=margin:5px;></i> COPY LEXPAD ID</a></li><li uib-tooltip=COLLAPSE tooltip-trigger=mouseenter tooltip-append-to-body=false tooltip-placement=right><a class=hovergold style=\"text-shadow:inset 0 0 1px rgba(0,0,0,0.5);\" title=\"collapse widget\" ng-show=!isCollapsed ng-click=\"isCollapsed = true\" onclick=\"$(\'.issuedocpanel\').addClass(\'closed\')\"><i class=\"fa fa-minus\" style=margin:5px;></i> COLLAPSE</a></li><li uib-tooltip=EXPAND tooltip-trigger=mouseenter tooltip-append-to-body=false tooltip-placement=right><a class=hovergold style=\"text-shadow:inset 0 0 1px rgba(0,0,0,0.5);\" title=\"expand widget\" ng-show=isCollapsed ng-click=\"isCollapsed = false\" onclick=\"$(\'.issuedocpanel\').removeClass(\'closed\')\"><i class=\"fa fa-plus\" style=margin:5px;></i> EXPAND</a></li><li uib-tooltip=\"COPY TO CLIPBOARD\" tooltip-trigger=mouseenter tooltip-append-to-body=false tooltip-placement=right><a class=hovergold style=\"text-shadow:inset 0 0 1px rgba(0,0,0,0.5);\" title=\"copy to clipboard\" ng-click=edit(roarevent.id)><i class=\"fa fa-copy\" style=margin:5px;></i> COPY TO CLIPBOARD</a></li><li uib-tooltip=FULLSCREEN tooltip-trigger=mouseenter tooltip-append-to-body=false tooltip-placement=right><a class=hovergold style=\"text-shadow:inset 0 0 1px rgba(0,0,0,0.5);\" title=\"fullscreen widget\" ng-click=openFullScreen(roarevent.$id) ng-show=\"options.maximizable || true\"><i class=\"fa fa-expand\" style=margin:5px;></i> FULLSCREEN</a></li><li class=divider></li><li uib-tooltip=CLOSE tooltip-trigger=mouseenter tooltip-append-to-body=false tooltip-placement=right><a class=\"hovergold text-danger\" style=\"text-shadow:inset 0 0 1px rgba(0,0,0,0.5);\" title=\"remove widget\" ng-click=remove() onclick=\"$(this).closest(\'.issuedocpanel\').remove()\"><i class=\"fa fa-close\" style=margin:5px;></i> CLOSE</a></li></ul></div>");
$templateCache.put("{widgetsPath}/getphd/src/titleTemplateMetadata.html","<div class=\"card clearfix img img-hover img-shadow\" collection={{roarevent.$id}} style=\"z-index:1;position:absolute;left:99.5%;top:0;min-width:550px;height: 90vh;overflow-y: auto;transition: all 1s ease-in-out;\" ng-class=\"{\'collapse\':!roarevent.coplapse, \'collapse in\':roarevent.coplapse}\"><form role=form name=form ng-submit=collection.$save()><div class=container><div class=col-sm-7><div class=form-group><h6>ID: {{collection.$id}}</h6><p class=btn clipboard text=\"\'/files/uploads/\'+collection.$id+\'.html\'\" on-click=\"alertify.success(\'/files/uploads/\'+{{collection.$id}}+\'.html\')\" style=padding:0px;>{{\'/files/uploads/\'+collection.$id+\'.html\'}}</p></div><div class=form-group style=height:40px;><div class=\"form-group col-sm-6\" style=padding:0px;><label style=\"color: #006699\" for=date>Date</label><br><input type=text id=date ng-model=collection.date placeholder=YYYY-MM-DD ng-pattern=dddd-dd-dd ng-model-options=\"{updateOn: \'blur\'}\"></div><div class=\"form-group col-sm-6\"><label style=\"color: #006699\" for=rid>ID#</label><br><input id=rid type=text ng-model=collection.rid placeholder=# ng-model-options=\"{updateOn: \'blur\'}\"></div></div><div class=form-group><label for=dashboardTitle style=\"color: #006699\">Title</label> <input type=text style=\"width: 100%\" id=dashboardTitle ng-model=collection.title placeholder=Title required> <input type=text style=\"width: 100%\" id=dashboardTitle2 ng-model=collection.title2 placeholder=\"Line 2\"></div><div class=form-group><label for=description style=\"color: #006699\">Description</label><br><textarea type=text rows=5 class=form-control id=description ng-model=collection.description ng-model-options=\"{updateOn: \'blur\'}\"></textarea></div><div class=form-group><label for=long_cite style=\"color: #006699\">Long Citation</label><br><input type=text style=\"width: 100%\" ng-model=collection.citation placeholder=\"Long Citation\" ng-model-options=\"{updateOn: \'blur\'}\"></div><div class=form-group><label for=short_cite style=\"color: #006699\">Short Citation</label><br><input type=text style=\"width: 100%\" ng-model=collection.short_cite placeholder=\"Short Citation\" ng-model-options=\"{updateOn: \'blur\'}\"></div><div class=form-group><label for=thumbnail style=\"color: #006699\">Notes</label><br><textarea rows=15 type=text style=\"width: 100%\" ng-model=collection.notes placeholder=\"Enter plaintext notes\" ng-model-options=\"{updateOn: \'default blur\'}\" class=form-control></textarea></div><div class=form-group><label for=roarlist style=\"color: #006699\">Linked Docs</label><br><div smartpanel={{collection.$id}}></div></div></div><div class=col-sm-5><div class=form-group style=height:175px;><roar-event id={{roarevent.$id}} style=max-height:175px;><uib-rating ng-model=roarevent.rate max=5></uib-rating></roar-event></div><div class=form-group><label style=\"color: #006699\" for=icon>Icon</label><br><select name=icon ng-model=collection.icon ng-options=\"icon.value as icon.label for icon in $root.ICONS\"></select><input type=submit class=\"btn btn-glass btn-primary btn-md pull-right\" value=SAVE></div><div class=form-group><label style=\"color: #006699\">Style</label><br><select ng-model=collection.styleClass ng-options=\"class.value as class.label for class in $root.ROARCLASSES\"></select></div><div class=form-group><label for=sortOrder style=color:#006699>Sort Order</label><br><input type=number style=width:100%; placeholder=# id=sortOrder ng-model=collection.sortOrder></div><div class=form-group><label style=\"color: #006699\">Document Code</label><br><input style=\"width: 100%\" ng-model=collection.doccode uib-typeahead=\"code.code as code.label for code in DOCNAMES | filter:$viewValue | limitTo:8\" typeahead-wait-ms=100></div><div class=form-group><label style=\"color: #006699\">Tags</label><br><input style=\"width: 100%\" ng-model=collection.matches ng-list=,></div><div class=form-group><label style=\"color: #006699\">Name</label><br><input type=text style=\"width: 100%\" ng-model=collection.name placeholder=name ng-model-options=\"{updateOn: \'blur\'}\"></div><div class=form-group><label style=\"color: #006699\">FileName</label><br><input type=text style=\"width: 100%\" ng-model=collection.filename placeholder=\"file name\" ng-model-options=\"{updateOn: \'blur\'}\"></div><div class=form-group><label style=color:#006699>WebSource/Original URL</label><br><input type=text style=\"width: 100%\" ng-model=collection.originalUrl placeholder=/path/to/url ng-model-options=\"{updateOn: \'blur\'}\" ng-readonly=\"collection.originalUrl !== (\'\'||null)\"></div><div class=form-group><label for=url style=\"color: #006699\" clipboard text=\"\'https://mozilla.github.io/pdf.js/web/viewer.html?file=\'\" on-click=\"alertify.success(\'https://mozilla.github.io/pdf.js/web/viewer.html?file=\')\" ng-click=\"collection.mediaUrl = \'https://mozilla.github.io/pdf.js/web/viewer.html?file=\'+collection.mediaUrl\">URL</label><br><input type=text style=\"width: 100%\" ng-model=collection.mediaUrl placeholder=\"Path to URL\" ng-model-options=\"{updateOn: \'blur\'}\" id=url></div><div class=form-group><label for=thumbnail style=\"color: #006699\">Thumbnail</label><br><input type=text style=\"width: 100%\" ng-model=collection.thumbnail placeholder=\"Path to URL\" ng-model-options=\"{updateOn: \'blur\'}\" id=thumbnail></div><div class=form-group><label for=background style=\"color: #006699\">Background Image</label><br><input type=text style=\"width: 100%\" ng-model=collection.background placeholder=\"Path to URL\" ng-model-options=\"{updateOn: \'blur\'}\" id=background></div><div class=form-group><label for=footer style=\"color: #006699\">Footer</label><br><input type=text style=\"width: 100%\" ng-model=collection.footer placeholder=footer ng-model-options=\"{updateOn: \'blur\'}\" id=footer></div><div class=form-group style=padding:0px;><label for=roarlist style=\"color: #006699\">Linked Docs <i class=badge>{{roare.length}}</i></label><ul ng-scrollbar style=\"height:300px;overflow-y:scroll;list-style: none;\"><li ng-repeat=\"roareventid in collection.roarlist as roare\" ffbase={{roareventid}}><roar-chip id={{roareventid}} roarevent=item></roar-chip></li></ul></div><div class=form-group><label for=links style=color:var(--applicantblue)>{{collection.links.length}} Links <button class=\"btn btn-success btn-xs fa fa-plus\" ng-click=\"collection.links.push({title: \'NewLink\', url: \'//\', target:\'_blank\'})\"></button></label><ul><li ng-repeat=\"link in collection.links track by $index\"><input type=text ng-model=link.title placeholder=title> <input type=text ng-model=link.url placeholder=URL><select ng-model=link.target placeholder=target><option label></option><option label=_blank value=_blank></option><option label=_self value=_self></option><option label=_parent value=_parent></option><option label=_top value=_top></option><option label=fframe value=fframe selected></option><option label=pop value=pop></option></select></li></ul></div><div class=form-group><label for=menus style=color:var(--applicantblue)>{{menus.length}} Menus <button class=\"btn btn-success btn-xs fa fa-plus\" ng-click=\"collection.menus.push({title: \'NewMenu\', icon: \'fa-circle\', items: [\'$ID\']})\"></button></label><ul ng-repeat=\"menu in collection.menus\"><label editable-text=menu.title>{{menu.title}}</label><select ng-model=menu.icon ng-options=\"icon.value as icon.label for icon in ICONS\"></select><li ng-repeat=\"link in menu.items track by $index\"><input type=text ng-model=link.$value placeholder=\"ID OPTIONAL\"> <button class=\"btn btn-xs text-success fa fa-circle-plus\" ng-click=\"menu.items.push(\'ROARID\')\"></button></li></ul></div></div></div></form></div>");
$templateCache.put("{widgetsPath}/getphd/src/uploader.html","<div class=\"card card-fancy card-rounded card-thick\"><h3 class=pull-right>Upload queue</h3><input type=file nv-file-select uploader=uploader multiple><p>Queue length: {{ uploader.queue.length }}</p><table class=table><thead><tr style=color:black;><th>Template</th><th width=50%>Name</th><th ng-show=uploader.isHTML5>Size</th><th ng-show=uploader.isHTML5>Progress</th><th>Status</th><th>Actions</th></tr></thead><tbody><tr ng-repeat=\"item in uploader.queue\" style=color:black; class={{item.styleClass}}><td><select ng-model=item.styleClass ng-options=\"class.value as class.label for class in $root.ROARCLASSES\"></select></td><td><strong>{{ item.file.name }}&nbsp;|&nbsp;{{item.file.type}}</strong></td><td ng-show=uploader.isHTML5 nowrap>{{ item.file.size/1024/1024|number:3 }} MB</td><td ng-show=uploader.isHTML5><div class=progress style=\"margin-bottom: 0;\"><div class=progress-bar role=progressbar ng-style=\"{\'width\': item.progress + \'%\' }\"></div></div></td><td class=text-center><span ng-show=item.isSuccess><i class=\"glyphicon glyphicon-ok\"></i></span> <span ng-show=item.isCancel><i class=\"glyphicon glyphicon-ban-circle\"></i></span> <span ng-show=item.isError><i class=\"glyphicon glyphicon-remove\"></i></span></td><td nowrap><button type=button class=\"btn btn-success btn-xs\" ng-click=item.upload() ng-disabled=\"item.isReady || item.isUploading || item.isSuccess\"><span class=\"glyphicon glyphicon-upload\"></span> Upload</button> <button type=button class=\"btn btn-warning btn-xs\" ng-click=item.cancel() ng-disabled=!item.isUploading><span class=\"glyphicon glyphicon-ban-circle\"></span> Cancel</button> <button type=button class=\"btn btn-danger btn-xs\" ng-click=item.remove()><span class=\"glyphicon glyphicon-trash\"></span> Remove</button></td></tr></tbody></table><div><div>Queue progress:<div class=progress><div class=progress-bar role=progressbar ng-style=\"{ \'width\': uploader.progress + \'%\' }\"></div></div></div><button type=button class=\"btn btn-success btn-s\" ng-click=uploader.uploadAll() ng-disabled=!uploader.getNotUploadedItems().length><span class=\"glyphicon glyphicon-upload\"></span> Upload all</button> <button type=button class=\"btn btn-warning btn-s\" ng-click=uploader.cancelAll() ng-disabled=!uploader.isUploading><span class=\"glyphicon glyphicon-ban-circle\"></span> Cancel all</button> <button type=button class=\"btn btn-danger btn-s\" ng-click=uploader.clearQueue() ng-disabled=!uploader.queue.length><span class=\"glyphicon glyphicon-trash\"></span> Remove all</button></div></div>");
...
$templateCache.put("{widgetsPath}/getphd/src/phd/docheader.html","<div class=\"container-fluid card bs two-col-right\"><div class=\"col-xs-12 col-sm-10 col-md-3 col-lg-4 card card-{{roarevent.styleClass}} col-1\" style=overflow-y:scroll;height:100%;><img dynamic-background background=url({{background(roarevent.styleClass)}}) background-position=\"0 0;\" background-repeat=no-repeat ng-src=\"{{roarevent.thumbnail || background(roarevent.styleClass)}}\" class=\"img img-responsive img-shadow img-thumbnail\" style=width:100%;max-width:100%;height:auto;margin:auto; ng-click=\"poodle = !poodle\"><div class=shelf><div class=bookend_left></div><div class=bookend_right></div><div class=reflection></div></div><hr><h4 class=hidden-md-up ng-click=\"roarevent.showframe = !roarevent.showframe\">{{roarevent.title}}</h4><p><label ng-repeat=\"match in roarevent.matches track by $index | groupBy: match\" class=\"fa fa-tag\">&nbsp; <a ng-click=getRelated(match)>{{match}}</a>&nbsp;</label></p><div class=hidden-md-up data-ng-transclude uib-collapse=roarevent.showframe allowfullscreen style=width:100%;height:80vh;overflow-y:auto;></div><iframe class=hidden-md-up ng-if=roarevent.showframe uib-collapse=!roarevent.showframe allowfullscreen name=fframe fullscreen={{full}} style=width:100%;height:80vh;min-height:80vh;overflow:scroll;padding:5px; ng-src=\"{{roarevent.mediaUrl || roarevent.media || \'/llp_core/img/lll3.svg\'}}\"></iframe><div ng-if=roarevent.roarlist uib-collapse=!poodle roarevents={{roarevent.$id}}><p ng-repeat=\"vent in roarevents | orderBy:[\'sortOrder\',\'date\',\'rid\']: maybe\" ffbase=\"{{vent.$id || vent.$value || vent}}\" style=max-width:100%;width:100%;height:50px;padding:0px; class=\"bs-callout-{{item.styleClass}} flex-row row\" ng-class=\"{\'bs-callout bs-callout-style bs-callout-reverse\': hoverover}\" ng-mouseenter=\"hoverover = true;\" ng-mouseleave=\"hoverover = false\" ng-click=getRelatedPage(vent)><roar-chip style=\"margin: 0px;\" id=\"{{vent.$id || vent.$value || vent}}\" roarevent=item></roar-chip><a ng-href=\"{{item.mediaUrl || item.media || \'/files/uploads/\'+item.$id+\'.html\'}}\" target=fframe title=\"{{item.filename || item.file.name || item.name || item.$id}}\" date={{item.date}} class=\"fa fa-3x fa-chevron-right fa-pull-right showonhover\" style=\"opacity: 0.05;margin-top:10px;\" uib-tooltip=\"view in fframe\" ng-click=\"roarevent.showframe = true\"></a></p></div></div><div class=\"col-md-9 col-lg-8 hidden-sm-down col-2\"><div class=\"bs-callout bs-callout-reverse bs-callout-{{roarevent.styleClass}}\"><h3 ng-dblclick=\"full = !full\" ng-click=\"roarevent.showframe = !roarevent.showframe\"><strong>{{roarevent.title}}</strong></h3><p ng-bind=roarevent.description></p><p>{{roarevent.date | date }}</p><cite class=\"pull-right clearfix\">{{roarevent.filename || roarevent.file.name || roarevent.name || roarevent.$id}} &nbsp;&nbsp; <a pop ng-href=\"{{roarevent.mediaUrl || roarevent.media || item.mediaUrl || item.media || collection.mediaUrl || collection.media}}\" title=\"{{roarevent.filename || roarevent.file.name || roarevent.name || roarevent.$id}}\" date=\"{{roarevent.date || item.date || collection.date}}\"><i class=\"fa fa-external-link\"></i></a></cite></div><iframe uib-collapse=!roarevent.showframe ng-if=roarevent.showframe allowfullscreen name=fframe fullscreen={{full}} style=width:100%;height:80vh;min-height:80vh;overflow:auto;padding:5px; ng-src=\"{{roarevent.mediaUrl || roarevent.media || \'/llp_core/img/lll3.svg\'}}\"></iframe><div data-ng-transclude uib-collapse=roarevent.showframe allowfullscreen style=\"width:100%;height:80vh;padding: 15px; overflow-y:scroll;\"></div></div></div>");
        ...
        return {
            ...
            link: function (scope, element, attr) {
                ...
                scope.$watch('textSize', function (size) {
                    angular.element('html').attr('style','font-size:'+ size + scope.unit + ' !important;');
                    $document.root.style.fontSize = size + scope.unit;
                    $('html').attr('style','font-size:'+ size + scope.unit + ' !important;');
                    alertify.log('font-size: ' + size + scope.unit)
                });
            }
        };
    }]);
