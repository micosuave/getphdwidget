...
angular.module("adf.widget.getphd").run(["$templateCache", function($templateCache) {$templateCache.put("{widgetsPath}/getphd/src/edit.html","<form role=form ng-submit=collection.$save()><fieldset class=\"material Applicant\"><input type=text placeholder=1234567 ng-model=config.pnum><hr><label>Enter a Patent Number</label></fieldset><fieldset class=\"material Petition\"><input type=text placeholder=1234567 ng-model=config.appnum><hr><label>Enter a Published Patent Application Number</label></fieldset></form>");
...
$templateCache.put("{widgetsPath}/getphd/src/phd/patentReport.html","<div class=\"container card-fancy\" style=width:98%;margin:1%;height:98%;overflow:scroll;><div class=\"col-xs-4 col-sm-3 col-1\"><div class=card><img ng-src={{patent.drawings[0]}} class=\"img img-responsive img-shadow img-thumbnail\" style=max-height:300px;width:auto;><div class=card-block><h2 class=\"card-title text-NOA\">US <a class=\"pop text-NOA\" ng-href={{patent.media}}><strong ng-bind=\"patent.id | number:0\"></strong><span class=\"fa fa-external-link\"></span></a><small class=pull-right>ISSUED {{patent.issued|date}}</small></h2><div class=row><em><label>USSN {{patent.application_number | application}}</label></em></div></div></div></div><div class=\"card col-xs-8 col-sm-9\"><div><uib-tabset class=\"panel panel-default panel-heading\"><uib-tab heading=Abstract><div class=\"bs-callout bs-callout-reverse bs-callout-NOA\"><h4 ng-bind=patent.title>Reprehenderit aute proident cupidatat exercitation officia incididunt culpa ullamco.</h4><p ng-bind=patent.abstract>Culpa enim minim amet proident sunt aliqua ex irure ex sunt eiusmod ut consectetur. Minim esse in tempor reprehenderit esse cupidatat adipisicing ipsum eiusmod. Ea commodo nisi enim esse et ut. Minim laborum irure eiusmod Lorem consequat duis labore deserunt ullamco velit enim ut. Aliquip tempor non amet aliqua cillum sit amet commodo aliqua sint nisi. Fugiat ex irure qui et in qui velit commodo ipsum. Non cupidatat laboris culpa ipsum culpa velit.</p></div></uib-tab><uib-tab><uib-tab-heading>Drawings <label class=\"label label-pill label-success\">{{patent.drawings.length}}</label></uib-tab-heading><div class=showscroll style=\"margin: 5px 5px;display:flex;flex-wrap:wrap;flex-direction:row;\"><a ng-repeat=\"link in patent.drawings\" pop=true ng-click=main.pop(link) class=btn style=margin:2px;><img ng-src={{patent.thumbnails[$index]}} class=\"img img-shadow\"></a></div></uib-tab><uib-tab heading=Description><pre class=\"card card-block\" style=\"text-size: 11px!important;\" ng-bind-html=\"patent.text | trustAsHTML\">\n                </pre></uib-tab><uib-tab><uib-tab-heading>Claims <label class=\"label label-pill label-info\">{{patent.claim_total}}</label></uib-tab-heading><div class=container><div class=col-xs-6><d3pendingtree patent={{patent.id}} pattern=\"{{query || \'\\s\'}}\" tree=scree></d3pendingtree><input id=patterninput ng-model=query type=search style=margin:10px; ng-model-options=\"{updateOn: \'default blur\',debounce: {\'default\':250,\'blur\':0}}\"><div class=col-sm-3><ul class=list-unstyled><li><input type=checkbox ng-model=dim> <small>Dim non-matches</small></li></ul></div><div class=col-sm-3></div></div><div class=col-xs-6><div id=info></div><div ui-tree=treeOptions><ol ui-tree-nodes max-depth=6 ng-model=patent.claims><li ui-tree-node class=\"card card-block\" ng-repeat=\"node in patent.claims\" ng-include=\"\'claim_renderer.html\'\" style=padding-right:0rem;padding-bottom:0.1rem; ng-hide=\"!treeFilter(node, query, supportedFields) && dim\"></li></ol></div><script type=text/ng-template id=claim_renderer.html><div ui-tree-handle class=\"tree-node tree-node-content\"> <div class=\"tree-node-content flextoprow \" style=\"position:relative;\"> <a class=\"btn btn-xs pull-left\" data-nodrag ng-click=\"toggle(this)\" ng-if=\"node.children && (node.children.length > 0)\"><span class=\"fa \" ng-class=\"{\'fa-chevron-right\': collapsed, \'fa-chevron-down\': !collapsed}\" style=\"color:steelblue;transition:all 0.25s ease;\"></span></a> <!--<input type=\"text\" ng-model=\"node.text\" ng-change=\"node.$save();\" ng-model-options=\"{ updateOn: \'default blur\', debounce: {\'default\': 1000, \'blur\': 0} }\" style=\"padding: 0.5rem;color:#444;\" ng-if=\"config.editable\">--> <!--<a class=\"btn showonhover\" data-nodrag ng-if=\"config.editable\" ng-click=\"remove(this);\"><span class=\"fa fa-close text-danger \"></span></a>--> <!--<a class=\"btn \" data-nodrag ng-if=\"config.editable\" ng-click=\"toc.newsubsection(this)\" style=\"\"><span class=\"fa fa-plus text-success\"></span></a>--> <div ng-bind-html=\"node.text | highlight: query \" ng-class=\"{\'filtered-out\':(!treeFilter(node, query, supportedFields) && dim)}\"></div> <!--<a class=\"gototarget btn\" data-nodrag ui-sref=\"{{parentstate}}.righttab({tabid: node.id})\" style=\"\"> <span ng-if=\"!config.editable\" class=\"pull-left\">{{node.text}}</span><i style=\"position:absolute;right:0;\">&nbsp;</i></a>--> </div> </div> <ol ui-tree-nodes=\"\" ng-model=\"node.children\" ng-class=\"{\'hidden\': collapsed}\" style=\"\"> <li class=\"card card-block img-shadow\" ng-repeat=\"node in node.children\" ui-tree-node ng-include=\"\'claim_renderer.html\'\" style=\"padding-right:0rem;padding-bottom:0.1rem;padding-left:5px;\" ng-hide=\"!treeFilter(node, query, supportedFields) && dim\" > </li> </ol></script></div></div></uib-tab><uib-tab heading=Citations><uib-tab-heading>Citations <label class=\"label label-pill label-warning\">{{patent.references.length}}</label></uib-tab-heading><table class=\"table table-responsive table-stripped table-condensed table-hover\"><tbody><tr ng-repeat=\"citation in patent.references\"><td><img ng-src=/patents/{{citation}}/preview class=\"img img-hover img-responsive img-shadow\" style=width:35px;height:35px;margin:5px;></td><td><em>{{citation}}</em></td><td col-span=2><patent-citation patent={{citation}}></patent-citation></td></tr></tbody></table></uib-tab></uib-tabset></div></div></div>");
